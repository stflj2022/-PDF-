...
class Layouter:
    ...
    def layout(self, characters: List[CharacterBox]) -> List[np.ndarray]:
        ...
        # Determine line height (max height of chars in batch or fixed?)
        # For simplicity, we can use a running max height for the current line
        # But to have consistent line spacing, we should perhaps pre-calculate or use the max of the current line.
        # Let's use a dynamic approach: accumulate chars for a line, then place them.
        
        # Better approach for streaming:
        # Just place them. If we hit the edge, move down.
        # We need to know the height of the line.
        # Let's estimate line height from the first few characters or use a fixed one if scaled.
        
        # Let's use the average scaled height of characters to estimate line advance
        avg_h = sum(c.height for c in characters) / len(characters)
        line_height = int(avg_h * self.config.output.scale_factor)
        ...
        # To handle line breaking correctly, we process character by character
        # But we need to resize them first to know their new dimensions
